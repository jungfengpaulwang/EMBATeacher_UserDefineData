    ...
    public partial class ExportStudentV2 : FISCA.Presentation.Controls.BaseForm, SmartSchool.API.PlugIn.Export.ExportWizard
    {
        ...
        public ExportStudentV2(string title, Image img)
        {
            ...
            _ExportableFields.ItemsChanged += delegate
            {
                ...
                List<string> newFields = new List<string>(new string[] { "姓名", "暱稱" });
        ...
        private void wizard1_FinishButtonClick(object sender, CancelEventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                #region 取得選取教師編號
                List<K12.Data.TeacherRecord> selectedStudents = K12.Data.Teacher.SelectByIDs(K12.Presentation.NLDPanels.Teacher.SelectedSource);
                foreach (K12.Data.TeacherRecord stu in selectedStudents)
                {
                ...
                #region 把全部教師以_PackageLimint人分一包
                int count = 0;
        ...
        void bkwNotPassComputer_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (ManualResetEvent eve in Filler.Keys)
            {
                ...
                if (RowIndex <= 65535)
                {
                    ...
                    {
                        K12.Data.TeacherRecord student = null;
                        if (row.ID != "")
                            student = K12.Data.Teacher.SelectByID(row.ID);
